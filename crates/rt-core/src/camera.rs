...
impl Camera {
    ...
    pub fn get_ray(&self, x: u32, y: u32, sample: u32, rng: &mut fastrand::Rng) -> Ray {
        debug_assert!(
            x < self.width,
        ...
        // Instante aleatorio del obturador: solo tiene efecto con objetos en movimiento
        let time = rng.f32();

        Ray { origin, direction, time }
    }

    fn sample_square(&self, rng: &mut fastrand::Rng) -> Vec3 {
    ...
    fn sample_disk_in_unit_circle(&self, rng: &mut fastrand::Rng) -> Vec3 {
        loop {
