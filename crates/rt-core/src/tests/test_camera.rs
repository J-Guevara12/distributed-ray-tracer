...
fn test_camera_center_ray_direction() {
    ...
    let ray = camera.get_ray(200, 112, 0, &mut fastrand::Rng::new());

    // La dirección debe apuntar hacia el frente (Z negativo)
    assert!(ray.direction.z < 0.0, "El rayo debe apuntar hacia adelante");
    assert!(
        ray.direction.x.abs() < 0.05,
        "El rayo central no debería desviarse mucho en X"
    );
}
...
fn test_camera_ray_normalization() {
    ...
    let rays = vec![
        camera.get_ray(0, 0, 0, &mut fastrand::Rng::new()),
        camera.get_ray(399, 0, 0, &mut fastrand::Rng::new()),
        camera.get_ray(0, 224, 0, &mut fastrand::Rng::new()),
        camera.get_ray(399, 224, 0, &mut fastrand::Rng::new()),
    ];
    ...
    for ray in rays {
        ...
        assert!(
            (length - 1.0).abs() < 1e-5,
            "La dirección del rayo de la cámara debe estar normalizada: {}",
            length
        );
    }
}
